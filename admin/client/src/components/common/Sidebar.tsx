...
const Sidebar: React.FC<{ activekey: any }> = (props) => {
  ...
  const openChildren = (id: any) => {
    ...
    if (otherTabs) {
      for (let i = 0; i < otherTabs.length; i++) {
        if (otherTabs[i].id !== id) {
          ...
          const otherTabsParent = otherTabs[i].parentElement as HTMLElement
          if (otherTabsParent.children.length > 1) {
            otherTabsParent.children[0].setAttribute("aria-expanded", "false")
          }
        }
      }
    }
    ...
    if (menutab) {
      const menutabParent = menutab.parentElement as HTMLElement
      if (menutab.classList.contains("show")) {
        menutab.classList.remove("show")
        if (menutabParent.children.length > 1) {
          menutabParent.children[0].setAttribute("aria-expanded", "false")
        }
      } else {
        menutab.classList.add("show")
        if (menutabParent.children.length > 1) {
          menutabParent.children[0].setAttribute("aria-expanded", "true")
        }
      }
    }
  }
  ...
  const GotoChangeMenu = (val: string) => {
    if (val === "UI Components") {
      // props.history.push("ui-alerts");
      navigate(`${process.env.REACT_APP_BASE_URL}/ui-alerts`)
      setMenuData([...menu2.menu2]);
    } else {
      // props.history.push("hr-dashboard");
      navigate(`${process.env.REACT_APP_BASE_URL}/hr-dashboard`)
  ...
  return (
    <div id="mainSideMenu" className={`sidebar px-4 py-4 py-md-5 me-0 ${isSidebarMini ? "sidebar-mini" : ""}`}>
      <div className="d-flex flex-column h-100">
        <a href={`${process.env.REACT_APP_BASE_URL}/community`} className="mb-0 brand-icon">
                    ...
                    <ul className="sub-menu collapse has-children" id={"menu-Pages" + i}>
                      {d.children.map((data, ind) => {
                        ...
                        return <li key={"jfdgj" + ind}>
                          <Link className={activekey === "/" + data.routerLink[0] ? "ms-link active" : "ms-link"} to={`${process.env.REACT_APP_BASE_URL}` + "/" + data.routerLink[0]}> <span>{data.name}</span></Link>
                        </li>
